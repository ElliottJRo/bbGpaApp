 ...
 */
#ifndef _COURSEMODEL_H
#define _COURSEMODEL_H
...
// The course model is based on the QListDataModel template, which in turn
// implements the abstract DataModel class.
typedef bb::cascades::QListDataModel<QVariantMap> CourseListModel;

/* CourseModel Description:
 *
 * Model class for the Course List application, the data model
 * reads and write from a JSON file that keeps all item data
 * for the list.
 */
class CourseModel: public CourseListModel
{
Q_OBJECT
 ...
 * kick the course.
 */
Q_PROPERTY(bool courseIsFull READ courseIsFull NOTIFY courseIsFullChanged)
...
public:
     ...
     *
     * @return The assets path to the JSON file for the course list app.
     */
    QString jsonAssetPath();

    /**
     * Sets the relative path to the JSON file containing the initial course list
     * data.
     *
    ...
    /**
     * This function returns the filled status of the course list data. The application is set
     * so that the user only will be allowed to have one hundred items in total.
     * When the course is full, false is returned.
     *
...
signals:
    ...
    /**
     * This signal is emitted when the course filled status is changed.
     *
     * @param courseIsFull True if the item limit of one hundred items is reached, otherwise false.
     */
    void courseIsFullChanged(bool courseIsFull);
...
public slots:
     ...
     */
    void deleteCourseItems(const QVariantList selectionList);
     ...
     */
    void addCourseItem(const QString itemTitle);
     ...
     */
    void editCourseItem(const QVariant item, const QString newItemTitle);
...
private:
    ...
    /**
     * This helper function is for saving the data in mCourseData to the JSON file.
     *
    ...
    /**
     * This helper function is for evaluating if the course is full. Alters the courseIsFull property
     * if the state needs changing and emits the corresponding signal.
     */
    void checkCourseIsFull();
    ...
    QVariantList mCourseData;

    // Constant set to the maximum number of course items (a hundred
    // things to do before you kick the course)
